...
class Factory extends MX_Controller {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New Nurse
                if ($this->ion_auth->email_check($email)) {
                    ...
                    redirect('factory-add');
                } else {
                    $dfg = 8;
                    $this->ion_auth->register($username, $password, $email, $dfg);
                    $ion_user_id = $this->db->get_where('users', array('email' => $email))->row()->id;
                    $this->nurse_model->insertNurse($data);
                    $nurse_user_id = $this->db->get_where('factory', array('email' => $email))->row()->id;
                ...
                $ion_user_id = $this->db->get_where('factory', array('id' => $id))->row()->ion_user_id;
                if (empty($password)) {
    ...
    function delete() {
        $data = array();
        $id = $this->input->get('id');
        $user_data = $this->db->get_where('factory', array('id' => $id))->row();
